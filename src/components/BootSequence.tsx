...
export const BootSequence: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[9999] bg-gray-50 text-cyan-500 font-mono flex flex-col justify-end p-8"
      style={{
        ...
        <div className="space-y-2 mb-8 h-48 overflow-hidden flex flex-col justify-end">
          {logs.map((log, i) => (
            ...
            >
              ...
              <span className={i === bootLogs.length - 1 ? 'text-blue-600 font-bold drop-shadow-[0_0_5px_rgba(240,199,34,0.8)]' : ''}>{log}</span>
            </motion.div>
          ))}
        </div>
