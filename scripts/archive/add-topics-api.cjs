...
let file = fs.readFileSync('src/lib/gemini.ts', 'utf8');

const newFunc = `
export async function generateDiverseTopics(mood: string, engine: string, onProgress?: any, signal?: AbortSignal, model?: string): Promise<{title: string, description: string}[]> {
  try {
...
if (!file.includes('generateDiverseTopics')) {
    file += newFunc;
    fs.writeFileSync('src/lib/gemini.ts', file);
    console.log("Added generateDiverseTopics");
}
